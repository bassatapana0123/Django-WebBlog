    ...
    if request.method == "POST":
        ...
        if username == "" or email == "" or password=="" or repassword == "":
            ...
            if password == repassword :
                #ตรวจสอบ Username ว่าซ้ำกันหรือไม่
                ...
                messages.info(request,"ไม่สามรถลงทะเบียนได้รหัสผ่านไม่ตรงกัน")
                return redirect("member")
...
def login(request):
    ...
    if user is not None:
        auth.login(request,user) #บันทึกการล็อคอินเพื่อใช้ในการตรวจสอบสำหรับ @login_required
